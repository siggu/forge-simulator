    ...
    <div className='bg-zinc-700 rounded-lg p-4 mt-4'>
      ...
      <div className='overflow-x-auto'>
        <table className='w-full text-sm'>
          <thead>
            <tr>
              <th className='px-2 py-1 border-b border-zinc-600'>강화 단계</th>
              {stoneAmounts.map((_, index) => (
                <th key={index} className='px-2 py-1 border-b border-zinc-600'>
                  {index}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            <tr>
              <td className='px-2 py-1 border-b border-zinc-600'>필요 강화석</td>
              {stoneAmounts.map((amount, index) => (
                <td key={index} className='px-2 py-1 border-b border-zinc-600 text-center'>
                  {amount}
                </td>
              ))}
            </tr>
